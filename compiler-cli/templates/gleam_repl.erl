...
-export([repl_save/1, repl_load/1, repl_print/1, repl_has_var/1]).
...
-define(VARS_KEY, gleam_repl_vars).

repl_save(Value) ->
    CurrentVars = case erlang:get(?VARS_KEY) of
        undefined -> [];
        List -> List
    end,

    NewVars = CurrentVars ++ [Value],
    erlang:put(?VARS_KEY, NewVars),
    Value.

repl_load(Index) ->
    case erlang:get(?VARS_KEY) of
        undefined ->
            not_found;
        List ->
            try
